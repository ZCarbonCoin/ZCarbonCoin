...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZCarbonCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "zcarboncoin.conf"));
...
{
    return "\n" +
           FormatParagraph(strprintf(_("In order to ensure you are adequately protecting your privacy when using ZCarbonCoin, please see <%s>."),
...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i ZCarbonCoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
